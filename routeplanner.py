...
def get_current_node(self):
    ...
    for node in self.openSet:
        currentl[node] = self.calculate_fscore(node)

    #minimum = min(current.keys(), key=lambda k: current[k])
    minimum = min(currentl, key=currentl.get)
...
def get_gScore(self, node):
    ...
    #    sum = distance(self,self.start, self.M.intersection[i])
    #    if len(self.cameFrom) == 1:
    #        sum = sum
    #  else:
            
     #       sum = sum + distance(self,i[0],i+1[0])

    #self.gScore[node] = sum
...
def heuristic_cost_estimate(self, node):
    ...
    heur = distance(self, node, self.goal)
...
def calculate_fscore(self, node):
    ...
    # REMEMBER F = G + H
    return self.gScore[node] + heuristic_cost_estimate(self,node)
...
def record_best_path_to(self, current, neighbor):
    ...
    #fScore[neighbor] := gScore[neighbor] + h(neighbor)  From wiki
...
# 
# **ANSWER**: A-star is a search algorithm that is provides the cheapest and shortest estimated path , by considering path cost (which means the amount of "work" you have to do if you choose a certain point) and heuristics (which means a form of estimation guide to ensure you dont deviate away from the goal)
# 
